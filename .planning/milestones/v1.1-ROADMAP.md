...
**Key Decisions:**
- pg_trgm similarity threshold 0.3 for names, 0.4 for addresses
- Rate limit reset at midnight KST for Korea relevance
- Server Actions preferred over API routes for forms
- Upsert pattern with ON CONFLICT for ratings
- CSS columns for masonry layout (no JS library)
- Optimistic UI with automatic rollback for voting
- forbidden() in layout for admin route protection
- Parallel Promise.all for dashboard data fetching

**Issues Resolved:**
- Edit submission page missing → implemented in 07-05-PLAN.md
- pg_trgm RPC for fuzzy matching → implemented in 07-06-PLAN.md
- Rating form not wired to Server Action → fixed in 08-05-PLAN.md
- getUserRating wrong table name → fixed in 08-05-PLAN.md

**Issues Deferred:**
- SUBMIT-07: User notification on approval/rejection → moved to v2

**Technical Debt Incurred:**
- None significant — all gap closures addressed during milestone
