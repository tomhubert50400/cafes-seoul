...
export function transformCafe(row: Record<string, unknown>): Cafe {
  return {
    ...
    ratings: {
      ...
      service: row.rating_service ? parseFloat(row.rating_service as string) : null,
...
export function transformCafeSummary(row: Record<string, unknown>): CafeSummary {
  return {
    ...
    photoUrls: (row.photo_urls as string[] | undefined) || undefined,
    operatingHours: row.operating_hours ? (row.operating_hours as OperatingHours) : undefined,
...
export function transformUserRating(row: Record<string, unknown>): UserRating {
  ...
  return {
    ...
    hasWifi: row.has_wifi as boolean || false,
    hasPowerOutlets: row.has_power_outlets as boolean || false,
    isLaptopFriendly: row.is_laptop_friendly as boolean || false,
