...
export function PhotoUpload({ cafeId, onUploadSuccess, disabled = false }: PhotoUploadProps) {
  ...
  const handleUpload = useCallback(async () => {
    ...
    // Call success callback if any succeeded
    if (successCount > 0) {
      onUploadSuccess?.();
    }
  ...
  return (
    <Card className="w-full max-w-lg mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          Share photos of this cafe. Photos will be reviewed before appearing publicly.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
            ...
            >
              <div className="flex flex-col items-center gap-2">
                ...
                <span className="text-sm text-muted-foreground">
                  Click to select photos (up to {maxAllowed})
                </span>
        ...
        >
          {state.isUploading ? (
            ...
            <>
              ...
              Upload {state.selectedFiles.length > 1 ? `${state.selectedFiles.length} Photos` : 'Photo'}
            </>
          )}
        </Button>
        ...
        <p className="text-xs text-muted-foreground text-center">
          By uploading, you agree that your photos will be reviewed for quality and
