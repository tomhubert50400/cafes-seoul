  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        {/* Name and district */}
        <div>
          <h3 className="font-semibold text-foreground line-clamp-1">{cafe.nameKo}</h3>
          {cafe.nameEn && (
            <p className="text-xs text-muted-foreground line-clamp-1">{cafe.nameEn}</p>
          )}
        </div>

        {/* Location */}
        <p className="text-sm text-muted-foreground line-clamp-1">
          {district?.nameKo || ''} · {cafe.addressKo.split(' ').slice(2, 4).join(' ')}
        </p>
